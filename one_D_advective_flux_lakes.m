function [T_source,S_source]=one_D_advective_flux_lakes(phi_i,S_i,T_i,...
...
alpha=(1/10000)*1.56*10^-1;                    % Linear coeff for Rayleigh number driven advection
%alpha=1.56*10^1;
...
for i=1:length(phi_i)
    if phi_i(i)>0.05
        pi_i(i)=(10^-17)*((10^3)*phi_i(i)).^3.1;          % permeability from GN (2013)
    else
        pi_i(i)=0;
    end
end
...
%         F_c(1:i)=0;
        ...
        F_c(i)=alpha*(Ra_i(i)-Ra_c)*(grid_size)*dt;
    end
end
...
for i=1:(ice_thickness/grid_size)+1;
    if i==1
        ...
        T_source(i)=c_br*(rho_sw+rho_sw*beta*(S_i(i)-S_sw))...
            *(grid_size)*(((F_in(i)*T_i(i+1)+(((rho_sw+rho_sw*beta*(S_i(i)-S_sw))...
            *(grid_size))-F_in(i))*T_i(i))/...
